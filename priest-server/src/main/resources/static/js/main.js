layui.use(['element', 'form', 'layer', 'upload'], function () {
    ...
    /* 最大化窗口 */
    $(".priest-screen-full").on("click", function (e) {
        e.preventDefault();
    ...
    $(".ajax-status").on("click", function (e) {
        e.preventDefault();
        let checked = [];
        let tdcheckbox = $(".priest-table td .priest-checkbox :checkbox:checked");
    ...
    // 展示数据列表-多选框
    let thcheckbox = $(".priest-table th .priest-checkbox :checkbox");
    thcheckbox.on("change", function () {
        let tdcheckbox = $(".priest-table td .priest-checkbox :checkbox");
    ...
    // 检测列表数据是否为空
    let timoTable = $(".priest-table tbody");
    if (timoTable.length > 0) {
        let children = timoTable.children();
        if (children.length === 0) {
            let length = $(".priest-table thead th").length;
            let trNullInfo = "<tr><td class='priest-table-null' colspan='"
    ...
    let paramSkip = function () {
        let getSearch = "";
        // 搜索框参数
        $('.priest-search-box [name]').each(function (key, val) {
    ...
    /* 展示列表数据搜索 */
    $(document).on("click", ".priest-search-btn", function () {
        paramSkip();
    });
    /* 改变显示页数 */
